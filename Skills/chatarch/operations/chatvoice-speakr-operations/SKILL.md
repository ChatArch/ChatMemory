...
- Normal automated tests must execute the actual production controllers and registered event handlers without a browser. Fake only device/socket/provider boundaries, then assert real state transitions, persistence/undo, busy/error recovery, track/socket/audio/timer cleanup and rejection of late events. Source-string assertions are supplementary, not operation coverage.
- Keep a durable action-to-test matrix and one normal test command for login/guest/token, meeting lifecycle, recorder/ASR, notes/title/revision/presets, TTS, clone and realtime logic. Exercise success, validation/auth errors, cancellation, duplicate operations and delayed results.
- A separate opt-in live acceptance command must drive actual HTTP/SSE/WebSocket routes with synthetic data, validate complete text/audio results and clean up owned test artifacts. Mark failed and blocked dependencies explicitly; mocks and HTTP 200 alone do not establish live readiness. Never use automatic paid fallbacks to make tests pass.
- A realtime model listed by `/models`, a successful WebSocket handshake, or `session.created` does not prove entitlement. Verify session configuration and response completion. Token Plan errors can be flat `{code, message}` objects without `type`; normalize known permission/quota errors safely, close both pumps, preserve their actionable message through client close, and mark live acceptance BLOCKED without a paid fallback.
- Browser use is reserved for explicit visual/physical-device acceptance. Do not repeatedly add ad-hoc browser simulations as the only way to discover business-logic failures. If a new browser failure reveals a missing deterministic case, reproduce it in the normal suite before fixing.
